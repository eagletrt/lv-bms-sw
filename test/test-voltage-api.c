...
void check_voltage_api_init(void) {
    volt voltages[DEFS_SERIES_COUNT] = { 0 };
    memset(voltage_handler.voltages, 0xFF, DEFS_SERIES_COUNT * sizeof(*voltage_handler.voltages));
    voltage_api_init();
    TEST_ASSERT_EQUAL_FLOAT_ARRAY(voltages, voltage_handler.voltages, DEFS_SERIES_COUNT);
}
...
void check_voltage_api_update_voltage_when_index_is_out_of_bounds(void) {
    volt voltages[DEFS_SERIES_COUNT] = { 0 };
    TEST_ASSERT_EQUAL_INT_MESSAGE(VOLTAGE_RC_OUT_OF_BOUNDS, voltage_api_update_voltage(DEFS_SERIES_COUNT, 7.f), "voltage_api_update_voltage returned a different value!");
    TEST_ASSERT_EQUAL_FLOAT_ARRAY_MESSAGE(voltages, voltage_handler.voltages, DEFS_SERIES_COUNT, "Previously stored values have been modified!");
}
...
void check_voltage_api_update_voltages_with_valid_parameters(void) {
    const volt voltages[DEFS_SERIES_COUNT] = { 3.2f, 3.3f, 3.4f, 3.5f, 3.6f, 3.7f };
    TEST_ASSERT_EQUAL_INT_MESSAGE(VOLTAGE_RC_OK, voltage_api_update_voltages(0U, voltages, DEFS_SERIES_COUNT), "voltage_api_update_voltages failed!");
    TEST_ASSERT_EQUAL_FLOAT_ARRAY_MESSAGE(voltages, voltage_handler.voltages, DEFS_SERIES_COUNT, "Stored voltages are different!");
}

void check_voltage_api_update_voltages_with_null_voltages(void) {
    volt voltages[DEFS_SERIES_COUNT] = { 0 };
    TEST_ASSERT_EQUAL_INT_MESSAGE(VOLTAGE_RC_NULL_POINTER, voltage_api_update_voltages(0U, NULL, DEFS_SERIES_COUNT), "voltage_api_update_voltages returned a different value!");
    TEST_ASSERT_EQUAL_FLOAT_ARRAY_MESSAGE(voltages, voltage_handler.voltages, DEFS_SERIES_COUNT, "Previously stored values have been modified!");
}
...
void check_voltage_api_update_voltages_when_index_is_out_of_bounds(void) {
    const volt VOLTAGES[DEFS_SERIES_COUNT] = { 3.2f, 3.3f, 3.4f, 3.5f, 3.6f, 3.7f };
    volt voltages[DEFS_SERIES_COUNT] = { 0 };
    TEST_ASSERT_EQUAL_INT_MESSAGE(VOLTAGE_RC_OUT_OF_BOUNDS, voltage_api_update_voltages(DEFS_SERIES_COUNT, VOLTAGES, DEFS_SERIES_COUNT), "voltage_api_update_voltages returned a different value!");
    TEST_ASSERT_EQUAL_FLOAT_ARRAY_MESSAGE(voltages, voltage_handler.voltages, DEFS_SERIES_COUNT, "Previously stored values have been modified!");
}
...
void check_voltage_api_get_min(void) {
    const volt voltages[DEFS_SERIES_COUNT] = { 3.2f, 3.3f, 3.4f, 3.5f, 3.6f, 3.7f };
    memcpy(voltage_handler.voltages, voltages, DEFS_SERIES_COUNT * sizeof(*voltages));
...
#define COUNT (3U)
    volt dump[COUNT] = { 0 };
    TEST_ASSERT_EQUAL_INT(VOLTAGE_RC_OUT_OF_BOUNDS, voltage_api_dump_voltages(dump, DEFS_SERIES_COUNT, COUNT));
#undef COUNT
}
